...
//		GUI_Line(3*i,flow_pot[i],3*i+3,flow_pot[i+1],13);
		...
		SampPoint[1][i*2] = 42*3-3*i;
		SampPoint[1][i*2+1] = TrumHeight-1-flow_pot[i];
		oled.Draw_Line(42*3-3*i,TrumHeight-1-flow_pot[i],42*3-3*i,0,color_half);
	}
	oled.Draw_LineS(SampPoint[0],43);
	oled.Draw_LineS(SampPoint[1],43);
}
...
void OLED_FFT::Display_Style5(void)
{
	...
	{
		Index = (float)i*180/100;
		Temp = 64 - (u16)(Device_Msg.fft[Index]/4);
		if(Temp < 2)
			Temp = 2;
		if(flow_pot[i]+10 < Temp)
			flow_pot[i] += 4;
		...
		SampPoint[0][i*2+1] = flow_pot[i];
		SampPoint[1][i*2] = 3*i;
		SampPoint[1][i*2+1] = - flow_pot[i] + 129;
